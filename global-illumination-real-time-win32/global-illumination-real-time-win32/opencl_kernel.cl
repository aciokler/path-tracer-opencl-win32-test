...
__constant int DIFFUSE = 0;
__constant int REFRACTIVE = 1;
__constant int SPECULAR = 2;
...
float3 trace(__constant Sphere* spheres, const Ray* camray, const int sphere_count, const int* seed0, const int* seed1) {
	...
	for (int bounces = 0; bounces < 8; bounces++) {
		...
		else if (hitsphere.material == SPECULAR) {
			ray.dir = normalize((float3)(ray.dir.x - normal.x * 2.0f * dot(normal, ray.dir), ray.dir.y - normal.y * 2.0f * dot(normal, ray.dir), ray.dir.z - normal.z * 2.0f * dot(normal, ray.dir)));
		}
		else if (hitsphere.material == REFRACTIVE) {
			ray.origin = hitpoint + normal_facing * 0.0000001f;

			bool into = dot(normal, normal_facing) > 0; /* is ray entering or leaving refractive material?*/
			float nc = 1.0f;  /* Index of Refraction air*/
			float nt = 1.5f;  /* Index of Refraction glass/water*/
			float nnt = into ? nc / nt : nt / nc;  /* IOR ratio of refractive materials*/
			float ddn = dot(ray.dir, normal_facing);
			float cos2t = 1.0f - nnt * nnt * (1.f - ddn * ddn);

			/* Ideal dielectric REFRACTION */
			float3 reflectionDir = normalize((float3)(ray.dir.x  - normal.x * 2.0f * dot(normal, ray.dir), ray.dir.y - normal.y * 2.0f * dot(normal, ray.dir), ray.dir.z - normal.z * 2.0f * dot(normal, ray.dir)));

			if (cos2t < 0.0f) {  /* Total internal reflection */
				ray.dir = reflectionDir;
			} else {
				float part3 = (into ? 1 : -1)*(ddn*nnt + sqrt(cos2t));
				float3 tdir = (float3)(ray.dir.x*nnt, ray.dir.y*nnt, ray.dir.z*nnt);
				tdir.x -= normal.x * part3;
				tdir.y -= normal.y * part3;
				tdir.z -= normal.z * part3;
				tdir = normalize(tdir);
				/*tdir.x = ray.dir.x*nnt - normal.x * part3);
				tdir.y = ray.dir.y*nnt - normal.y * part3);
				tdir.z = ray.dir.z*nnt - normal.z * part3);
				tdir = normalize(tdir);*/
				...
				float RP = Re / P;
				float TP = Tr / (1.f - P);
				if (rand1 < P) { /* choose between reflection or transmition  */
					/* relfection */
					mask.x *= RP;
					mask.y *= RP;
					mask.z *= RP;
